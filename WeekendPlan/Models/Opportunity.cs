...
namespace WeekendPlan.Models
{
    public class Opportunity
    {
            ...
            int typeVacation, int count)
        {
            ...
            //var DistinctItems = listShows.GroupBy(x => x.ShowId).Select(y => y.First());
        ...
        private static List<Opportunity> ConvertFilmToOpportunity(List<Show> shows)
        {
            ...
            foreach (var e in s)
            {
                if (result.Count == 0)
                {
                    ...
                    opportunity.Image = (Film.GetFilms().Find(x => x.FilmId == e.MovieId).Images.Any())? Film.GetFilms().Find(x => x.FilmId == e.MovieId).Images[0].ToString():"";
        ...
        private static List<Opportunity> ConvertEventToOpportunity(List<Event> events)
        {
            ...
            foreach (var e in events)
            {
                if (result.Count == 0)
                {
                    ...
                    opportunity.Image = (e.Images.Any())? e.Images[0].ToString():"";
